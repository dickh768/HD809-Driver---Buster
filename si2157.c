...
static int si2157_init(struct dvb_frontend *fe)
{
	...
	if (ret) {
		dev_dbg(&client->dev, "firmware file '%s' not found\n",
				fw_name);
		goto err;
	}
	...
	if (fw->size % 17 != 0) {
		dev_dbg(&client->dev, "firmware file '%s' is invalid\n",
				fw_name);
		ret = -EINVAL;
	...
	for (remaining = fw->size; remaining > 0; remaining -= 17) {
		...
		if (len > SI2157_ARGLEN) {
			dev_dbg(&client->dev, "Bad firmware length\n");
			ret = -EINVAL;
			goto err_release_firmware;
		}
...
/*      return here to disable the standby command for testing purposes 
	return 0;   */
...
static int si2157_set_params(struct dvb_frontend *fe)
{
	...
	if (dev->chiptype == SI2157_CHIPTYPE_SI2146)
		...
		memcpy(cmd.args, "\x14\x00\x11\x07\x01\x00", 6);
	...
	memcpy(cmd.args, "\x14\x00\x11\x07\x03\x00", 6);
...
static void si2157_stat_work(struct work_struct *work)
{
	...
	goto temp1;   /*  temporary bypass of the periodic error gathering  */
			...
			const struct i2c_device_id *id)
{
	...
	if (!dev) {
		ret = -ENOMEM;
		dev_dbg(&client->dev, "kzalloc() failed6\n");
		goto err;
	}
