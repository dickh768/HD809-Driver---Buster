...
static int si2168_set_frontend(struct dvb_frontend *fe)
{
	...
	int ret, proto;
	struct si2168_cmd cmd;
	u8 bandwidth, delivery_system, lastval=0;
	...
	cmd_init(&cmd, "\x14\x00\x0A\x10\xF8\x02", 6, 4);
	ret = si2168_cmd_execute(client, &cmd);
	if (ret)
		goto err;
	...
	msleep(100);    //give the system time to settle
	
	//cycle through stored protocol combinations to find one which works
	
	delivery_system = 0;
	for (proto = 1; proto <= 250; ++proto) {

	// increment protocol selection on chip
		cmd_init(&cmd, "\x87\x01", 2, 8);
		ret = si2168_cmd_execute(client, &cmd);
		if (ret)
			goto err;

	//check return value for recognised protocol
	if((cmd.args[1] == 0) & (cmd.args[2] == 14) & (cmd.args[3] == 2)) delivery_system = SYS_DVBT;  
	if((cmd.args[1] == 0) & (cmd.args[2] == 14) & (cmd.args[3] == 7)) delivery_system = SYS_DVBT2;
		
	if(delivery_system | ((cmd.args[3] - lastval) == 32)) break;
	...
	//at this point the video should be available for streaming
	
	if(delivery_system) {
	

	cmd_init(&cmd, "\x82\x01", 2, 3);
	ret = si2168_cmd_execute(client, &cmd);
	if (ret)
		goto err;	
...
static int si2168_init(struct dvb_frontend *fe)
{
	...
	cmd_init(&cmd, "\xc0\x12\x00\x00\x00\x0d\x16\x00\x00\x00\x00\x00\x00",
		 13, 0);
	ret = si2168_cmd_execute(client, &cmd);
	if (ret)
		goto err;
	...
	if (ret) {
		dev_dbg(&client->dev, "firmware download failed %d\n", ret);
		goto err_release_firmware;
	}
	...
	// Commands copied from Cidana app
	...
	cmd_init(&cmd, "\x88\x01\x03\x01\x01", 5, 5);
	ret = si2168_cmd_execute(client, &cmd);
	if (ret)
		goto err;		
	
	cmd_init(&cmd, "\x89\x00\x06\x12\x00\x00", 6, 3);
	ret = si2168_cmd_execute(client, &cmd);
	if (ret)
		goto err;	
	...
	cmd_init(&cmd, "\x14\x00\x03\x10\x85\x00", 6, 4);
	ret = si2168_cmd_execute(client, &cmd);
	if (ret)
		goto err;	
	
	cmd_init(&cmd, "\x14\x00\x07\x10\x00\x20", 6, 4);
	ret = si2168_cmd_execute(client, &cmd);
	if (ret)
		goto err;	
	
	cmd_init(&cmd, "\x14\x00\x0F\x10\x1E\x00", 6, 4);
	ret = si2168_cmd_execute(client, &cmd);
	if (ret)
		goto err;	
	
	cmd_init(&cmd, "\x14\x00\x01\x10\x06\x00", 6, 4);
	ret = si2168_cmd_execute(client, &cmd);
	if (ret)
		goto err;	
	
	cmd_init(&cmd, "\x14\x00\x09\x10\x04\x11", 6, 4);
	ret = si2168_cmd_execute(client, &cmd);
	if (ret)
		goto err;

	cmd_init(&cmd, "\x14\x00\x08\x10\xC7\x11", 6, 4);
	ret = si2168_cmd_execute(client, &cmd);
	if (ret)
		goto err;
	...
	cmd_init(&cmd, "\x8E\x00\x23\x01\x00\x00\x00\x00", 8, 1);
	ret = si2168_cmd_execute(client, &cmd);
	if (ret)
		goto err;
...
err_kfree:
	kfree(dev);
err:
	dev_warn(&client->dev, "probe failed8 = %d\n", ret);
	return ret;
}
